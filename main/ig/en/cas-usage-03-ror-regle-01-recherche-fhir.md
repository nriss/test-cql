# 03 — ROR sans CQL - POC CQL - ROR v0.1.0
...
**Objectif** : implémenter exactement la même règle que le [cas d'usage 02](cas-usage-02-ror.md) — identifier les `Organization` non conformes à la règle de nommage "urgence" ([issue #1](https://github.com/nriss/test-cql/issues/1)) — mais **sans CQL**, uniquement avec des requêtes de recherche FHIR standard (chaînage inversé `_has`, `_summary=count`, modificateur `:contains`) et un post-traitement minimal côté client.
